...
Global reschedulers must have low overhead in to decrease application time. % sentence does not give the full picture. algorithms must be effective in order to improve scheduling, but also low overhead in order to avoid reducing its benefits
...
In this work we present the \textit{PackDrop} strategy, a refinement-based technique that behaves similarly to \textit{Grapevine}, implementing our new approach. % Saying that it is similar to Grapevine here is not helping to sell the strategy
...
Alongside with that, \textit{PackDrop} explores locality of tasks mapped to the same PE, since migrations are done in groups. % enumerate expected benefits to highlight them
...
With this information, each PE will take the least loaded task within its pool, and pack it (lines~$3-6$). % task have load, they are not loaded
...
The PackDrop strategy is formally presented in Algorithm~\ref{algo:packdrop}. % What does it mean formally?

...
\todo[inline]{I've changed the name of the platforms because both of them are clusters. Moreover, I've changed the order they appear in Table~\ref{tab:ptinfo} since we first present results for the Grid'5000. Please confirm the number of nodes in each cluster in Table~\ref{tab:ptinfo} and complete the information concerning the MPI version in Platform 1.}
...
\todo[inline]{If you decide to stay with tables instead of graphs, you should update the style of Table \ref{tab:eval:g5k:leanmd:time}.}
...
\todo[inline]{It seems that these graphs have different font sizes.}
...
\refinelb was excluded from this evaluation since \textit{LeanMD} input presents more data than \refinelb is able to process in a reasonable time, being in average $1,000$ms slower than \greedylb in the $384$ PEs test case. % Nenhuma ideia de tempo geral? O que é razoável?

\distributedlb benefits from this platform due to Infiniband low latency communication costs, which reflects on improved total application times, as seen in Figure~\ref{fig:eval:sdumont:leanmd:apptime}.
